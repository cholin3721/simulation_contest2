...
| 제함기 3대 · 30 box/min | Erector 2초/box (=30/min), capacity 1 | OK |
...
> **이론값 vs 실측(해결):** 문제 수정본의 '빈 주문 박스 → S등급 부여' 규칙 반영 시 이론 P(S/A) = 0.75 + (0.5×0.5×0.7×0.95) = **91.6%**로, 실제 DB **91.7%(8,120/8,858)**와 일치. DB가 문제 조건대로 생성됨을 검증 완료(2026-07-12). 모든 문서 서술 갱신됨.
...
|---|---|---|---|
| 완료 팔레트 | 8 | **20** | ×2.5 |
| 완료 박스 | 256 | **640** | +384 (**+150%**) |
| 시간당 처리량 | 1.6/hr | **4.0/hr** | ×2.6 |
| 완료 지점 | 1/60 | **3/60** | ×3 |
| 완료율 | 2.9% | **7.2%** | — |
| AGV 평균 가동률 | 23% | **50%** | ×2.2 |
| 제함기1 / 제함기2 | 100% / 0% | **100% / 4.8%** | 분산 제한적(하류 정체 역전파) |
...
- 표 8 (AS-IS vs TO-BE): 8팔레트(256박스) → **21팔레트(640박스)**, 1/60 → 3/60, 1.6 → **4.0/hr**, AGV 23% → **50%**, 제함기1/2 = **약 30% / 약 70%**
- 표 9 (민감도): 25대(TO-BE) = **20팔레트 · 4.0/hr · 약 50%**
